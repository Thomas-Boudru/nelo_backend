...
router.post("/signup", limiter, async (req, res) => {
  try {
    ...
    let pictureProfile =
      "https://res.cloudinary.com/dqr6dghcl/image/upload/v1709972669/Coinpack/Group_46_1_isxhjs.png";
...
router.post("/login", limiter, (req, res) => {
  ...
  const query = {
    $or: [{ email: req.body.email }, { "userData.pseudo": req.body.email }],
    automaticCreated: false,
  };
  ...
  User.find(query)
    .then((users) => {
      ...
      if (
        openUser &&
        bcrypt.compareSync(req.body.password, openUser.password)
      ) {
        return res.json({
          result: true,
        ...
        return res.json({
          result: false,
          error: "Email, pseudo, or password incorrect",
        });
      }
    })
    .catch((error) => {
    ...
    .then((data) => {
      if (data) {
        ...
        res.json({
          ...
          message: "User found",
          db: data,
          saldoOthersData: filteredSaldoOthersData,
        });
      } else {
      ...
      populate: [
        {
          path: "event",
          model: "events",
          select: "nameEvent pictureEvent timezone",
        },
        {
          path: "saldo",
          model: "saldos",
...
router.post("/closeAccount", limiter, (req, res) => {
  ...
  User.findOneAndUpdate({ token: tokenUser }, { isOpen: false }, { new: true })
    .then((updatedUser) => {
      if (updatedUser) {
        return res.json({
          result: true,
...
router.post("/sendNewPassword", limiter, (req, res) => {
  User.findOne({ email: req.body.email }).then((data) => {
    if (!data) {
      ...
      User.updateOne({ email: req.body.email }, { password: hash }).then(() => {
...
router.post("/checkPseudo", limiter, (req, res) => {
  // On supprime les espaces avant et arrière du pseudo avec trim()
  const pseudo = req.body.pseudo.trim();

  User.findOne({ "userData.pseudo": pseudo }).then((data) => {
    if (data) {
...
router.get("/checkNumber", limiter, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ result: false, message: "Error checking user count" });
  }
});
...
router.post("/toggleFavoriteEvent", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      result: true,
      message: "Favorite events updated",
      favoriteEvents: populatedUser.favoriteEvents,
    });
  } catch (error) {
    ...
    return res
      ...
      .json({
        result: false,
        error: "An error occurred while toggling favorite event",
      });
  }
});
...
router.post("/getFavoriteEvents", async (req, res) => {
  ...
  try {
    ...
    res.json({
      result: false,
      error: "An error occurred while fetching favorite events",
    });
  }
});
